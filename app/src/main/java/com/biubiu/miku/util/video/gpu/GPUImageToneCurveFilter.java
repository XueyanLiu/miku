...
package com.biubiu.miku.util.video.gpu;
...
public class GPUImageToneCurveFilter extends GPUImageFilter {
  ...
  public GPUImageToneCurveFilter() {
    ...
    PointF[] defaultCurvePoints =
        new PointF[] {new PointF(0.0f, 0.0f), new PointF(0.5f, 0.5f), new PointF(1.0f, 1.0f)};
  ...
  public void onInit() {
    super.onInit();
    mToneCurveTextureUniformLocation =
        GLES20.glGetUniformLocation(getProgram(), "toneCurveTexture");
  ...
  public void setFromCurveFileInputStream(InputStream input) {
    try {
      ...
      for (int i = 0; i < totalCurves; i++) {
        ...
        // Curves dialog graph) and the second is the input value. All coordinates have range 0 to
        // 255.
  ...
  public static FilterCurves getFilterCurvesFromInputStream(InputStream input) {
    try {
      int version = readShort(input);
      int totalCurves = readShort(input);
      ...
      PointF[] mRgbCompositeControlPoints = curves.get(0);
      PointF[] mRedControlPoints = curves.get(1);
      PointF[] mGreenControlPoints = curves.get(2);
      PointF[] mBlueControlPoints = curves.get(3);
      return new FilterCurves(mRgbCompositeControlPoints, mRedControlPoints, mGreenControlPoints,
          mBlueControlPoints);
    } catch (IOException e) {
      e.printStackTrace();
    }
      ...
      public void run() {
        ...
        if ((mRedCurve.size() >= 256) && (mGreenCurve.size() >= 256) && (mBlueCurve.size() >= 256)
          ...
          for (int currentCurveIndex = 0; currentCurveIndex < 256; currentCurveIndex++) {
            ...
            toneCurveByteArray[currentCurveIndex * 4] =
                (byte) ((int) Math.min(
                    Math.max(currentCurveIndex + mBlueCurve.get(currentCurveIndex)
                        + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
            toneCurveByteArray[currentCurveIndex * 4 + 1] =
                (byte) ((int) Math.min(
                    Math.max(currentCurveIndex + mGreenCurve.get(currentCurveIndex)
                        + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
            toneCurveByteArray[currentCurveIndex * 4 + 2] =
                (byte) ((int) Math.min(
                    Math.max(currentCurveIndex + mRedCurve.get(currentCurveIndex)
                        + mRgbCompositeCurve.get(currentCurveIndex), 0), 255) & 0xff);
  ...
  private ArrayList<Float> createSplineCurve(PointF[] points) {
    ...
    for (Point newPoint : splinePoints) {
      ...
      float distance =
          (float) Math.sqrt(Math.pow((origPoint.x - newPoint.x), 2.0)
              + Math.pow((origPoint.y - newPoint.y), 2.0));
