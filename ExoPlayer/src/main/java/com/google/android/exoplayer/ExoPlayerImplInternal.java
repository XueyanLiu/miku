...
/* package */ final class ExoPlayerImplInternal implements Handler.Callback {
  ...
  private static final int MSG_VOLUME = 2000;
  ...
  public long getBufferedPosition() {
    return bufferedPositionUs == TrackRenderer.UNKNOWN_TIME_US
        ? ExoPlayer.UNKNOWN_TIME
        : bufferedPositionUs / 1000;
  }

  public long getDuration() {
    return durationUs == TrackRenderer.UNKNOWN_TIME_US
        ? ExoPlayer.UNKNOWN_TIME
        : durationUs / 1000;
  }
  ...
  public void setVolume(float volume) {
    handler.obtainMessage(MSG_VOLUME, new Float(volume)).sendToTarget();
  }
  ...
  public boolean handleMessage(Message msg) {
    try {
      switch (msg.what) {
        ...
        case MSG_VOLUME: {
          setPlaybackVolumeInternal(((Float) msg.obj).floatValue());
          return true;
        }
        default:
          return false;
  ...
  private void updatePositionUs() {
    ...
    // Log.e("updatePositionUs","positionUs:"+(positionUs/1000)+"
  ...
  private void setPlaybackVolumeInternal(float volume) {
    for (int i = 0; i < enabledRenderers.size(); i++) {
      TrackRenderer renderer = enabledRenderers.get(i);
