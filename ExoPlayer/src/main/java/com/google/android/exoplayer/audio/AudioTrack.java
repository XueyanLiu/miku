...
public final class AudioTrack {
  ...
  // This should actually better be named 'blockingTemporaryBuffer'. But I keep it this
    ...
    if (enablePreV21AudioSessionWorkaround) {
      if (Util.SDK_INT < 21) {
        ...
        if (keepSessionIdAudioTrack == null) {
          ...
          keepSessionIdAudioTrack = new android.media.AudioTrack(streamType,
  ...
  public int handleBuffer(ByteBuffer buffer, int offset, int size, long presentationTimeUs)
    ...
    if (sonic == null) {
      ...
      if (channelConfig == AudioFormat.CHANNEL_OUT_MONO) {
        ...
        numChannels = 1;
        Log.e(TAG, "Surround channels are not supported at this time (Channelconfig: + "
            + channelConfig + ")");
      }
    ...
    if (!isAc3 && systemClockUs - lastTimestampSampleTimeUs >= MIN_TIMESTAMP_SAMPLE_INTERVAL_US) {
      ...
      if (audioTimestampSet) {
        ...
        if (audioTimestampUs < resumeSystemTimeUs) {
          ...
          Log.w(TAG, message);
          audioTimestampSet = false;
        } else if (Math.abs(framesToDurationUs(audioTimestampFramePosition)
            - playbackPositionUs) > MAX_AUDIO_TIMESTAMP_OFFSET_US) {
  ...
  private float bytesToFrames(float byteCount) {
    if (isAc3) {
      return ac3Bitrate == UNKNOWN_AC3_BITRATE
          ? 0L
          : byteCount * 8 * sampleRate / (1000 * ac3Bitrate);
    } else {
